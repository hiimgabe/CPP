...
/*   Updated: 2024/07/23 13:36:22 by gabe             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	PhoneBook::searchContact(void)
{
	...
	while (42)
	{
		std::cout << YELLOW << "Index input : ";
		std::cin >> id;
		if (std::cin.fail())
		{
			if (std::cin.eof())
				break ;
			PhoneBook::printError("Please input a number.");
			std::cin.clear();
			std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
		}
		else if (id < 0 || id > 7)
		{
			PhoneBook::printError("Invalid index input.\nOnly 0 to 7.");
		}
		else
		{
			_contacts[id].display();
			std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
			break ;
		}
	}
